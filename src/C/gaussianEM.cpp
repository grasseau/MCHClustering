...
void computeDiscretizedGaussian2D( const double *xyInfSup, const double *theta, 
    ...
    const double *varX_ = getConstVarX(theta, K);
    const double *varY_ = getConstVarY(theta, K);
    const double *muX  = getConstMuX(theta, K);
    const double *muY  = getConstMuY(theta, K);
    
    // ???
    double varX[K], varY[K];
    // vectorMultScalar( varX_, 0.78, K, varX);
    // vectorMultScalar( varY_, 0.78, K, varY);
    vectorMultScalar( varX_, 1.0, K, varX);
    vectorMultScalar( varY_, 1.0, K, varY);
