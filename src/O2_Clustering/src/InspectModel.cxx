...
/* Inv
static InspectModel inspectModel={.nbrOfProjPads=0, .projectedPads=0, .projGroups=0, .thetaInit=0, .kThetaInit=0,
...
void saveThetaEMInGroupList( const double *thetaEM, int K) {
  int element_size = 5;
  copyInGroupList( thetaEM, K, element_size, inspectModel.subClusterThetaEMFinal );
}

void saveThetaExtraInGroupList( const double *thetaExtra, int K) {
  int element_size = 5;
  copyInGroupList( thetaExtra, K, element_size, inspectModel.subClusterThetaExtra );
}

void saveThetaFitInGroupList( const double *thetaFit, int K) {
  int element_size = 5;
  copyInGroupList( thetaFit, K, element_size, inspectModel.subClusterThetaFitList );
}
...
void collectTheta( double *theta, Groups_t *thetaToGroup, int K) {
  ...
  for (int h=0; h < inspectModel.subClusterThetaFitList.size(); h++) {
    ...
    o2::mch::printTheta("collectTheta", inspectModel.subClusterThetaFitList[h].second, inspectModel.subClusterThetaFitList[h].first);
    o2::mch::copyTheta( inspectModel.subClusterThetaFitList[h].second, k, &theta[sumK], K, k );
    if ( thetaToGroup ) {
      o2::mch::vectorSetShort( &thetaToGroup[sumK], h+1, k);
    }
    sumK += k;
    if (VERBOSE) {
        printf("collect theta grp=%d,  grpSize=%d, adress=%p\n", h, k, inspectModel.subClusterThetaFitList[h].second);
    }
...
void cleanInspectModel( ) {
  ...
  inspectModel.subClusterThetaFitList.clear();
  //
  if ( inspectModel.projectedPads != nullptr) { delete[] inspectModel.projectedPads; inspectModel.projectedPads=nullptr; }
  if ( inspectModel.qProj != nullptr) { delete[] inspectModel.qProj; inspectModel.qProj= nullptr; }
  if ( inspectModel.projGroups != nullptr) { delete[] inspectModel.projGroups; inspectModel.projGroups=nullptr; }
  if ( inspectModel.thetaInit != nullptr) { delete[] inspectModel.thetaInit; inspectModel.thetaInit = nullptr;}
  //
...
int getNbrOfThetaExtra() {
  return inspectModel.totalNbrOfSubClusterThetaExtra;
}
...
void collectProjectedPads(double *xyDxy, double *chA, double *chB) {
  ...
  for ( int i=0; i < 4; i++) {
    for( int k=0; k < nbrOfProjPads; k++) xyDxy[i*nbrOfProjPads + k] = inspectModel.projectedPads[i*nbrOfProjPads + k];
  }
  for( int k=0; k < nbrOfProjPads; k++) chA[k] = inspectModel.qProj[k];
  for( int k=0; k < nbrOfProjPads; k++) chB[k] = inspectModel.qProj[k];
...
void saveProjPadToGroups( Groups_t *projPadToGrp, int N ) {
    inspectModel.projGroups = new Groups_t[N];
...
void savePadToCathGroup( const Groups_t *cath0Grp, const PadIdx_t *mapCath0PadIdxToPadIdx, int nCath0,
        const Groups_t *cath1Grp, const PadIdx_t *mapCath1PadIdxToPadIdx, int nCath1) {
  inspectModel.padToCathGrp = new Groups_t[nCath0+nCath1];
  if( cath0Grp != nullptr ) {
  ...
  if( cath1Grp != nullptr ) {
    for (int p=0; p < nCath1; p++ ) {
      // printf("savePadToCathGroup p[cath1 idx]=%d mapCath1PadIdxToPadIdx[p]= %d, grp=%d\n", p, mapCath1PadIdxToPadIdx[p], cath1Grp[p]);
...
void collectPadsAndCharges( double *xyDxy, double *z, Groups_t *padToGroup, int N) {
  ...
  for (int h=0; h < inspectModel.subClusterPadList.size(); h++) {
    int n = inspectModel.subClusterPadList[h].first;
    o2::mch::copyXYdXY( inspectModel.subClusterPadList[h].second, n, &xyDxy[sumN], N, n );
    o2::mch::vectorCopy( inspectModel.subClusterChargeList[h].second, n, &z[sumN]);
    if ( padToGroup ) {
...
void collectThetaFit( double *thetaFit, int K) {
  ...
  for (int h=0; h < inspectModel.subClusterThetaFitList.size(); h++) {
    int k = inspectModel.subClusterThetaFitList[h].first;
    o2::mch::copyTheta( inspectModel.subClusterThetaFitList[h].second, k, &thetaFit[sumK], K, k );
    sumK += k;
    delete[] inspectModel.subClusterThetaFitList[h].second;
  }
  if ( sumK > K) {
...
void collectThetaEMFinal( double *thetaEM, int K) {
  ...
  for (int h=0; h < inspectModel.subClusterThetaEMFinal.size(); h++) {
    int k = inspectModel.subClusterThetaEMFinal[h].first;
    o2::mch::copyTheta( inspectModel.subClusterThetaEMFinal[h].second, k, &thetaEM[sumK], K, k );
    sumK += k;
...
void collectThetaExtra( double *thetaExtra, int K) {
  ...
  for (int h=0; h < inspectModel.subClusterThetaExtra.size(); h++) {
    int k = inspectModel.subClusterThetaExtra[h].first;
    o2::mch::copyTheta( inspectModel.subClusterThetaExtra[h].second, k, &thetaExtra[sumK], K, k );
    sumK += k;
...
void setNbrProjectedPads( int n) { inspectModel.nbrOfProjPads = n;
