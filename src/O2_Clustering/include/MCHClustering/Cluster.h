...
class Cluster
{
 public:
  ...
  ~Cluster();
  inline int getNbrOfPads(int c) { return ( pads[c]==nullptr ? -1 : pads[c]->getNbrOfPads()); };
  ...
  inline int getNbrOfProjectedPads( ) { return (projectedPads==nullptr ? -1 : projectedPads->getNbrOfPads()); };
  // Unused - Old version
 ...
 private:
  ...
  void computeProjectedPads(
            const Pads &pad0InfSup, const Pads &pad1InfSup, int maxNbrOfProjPads,
