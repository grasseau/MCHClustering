...
private:
  static constexpr double SDistancePrecision =
      1.e-3; ///< precision used to check overlaps and so on (cm)
  ...
  void buildPixArray();
  void ProjectPadOverPixels(const PadOriginal &pad, TH2D &hCharges,
                            TH2I &hEntries) const;
  ...
  void process();
  void addVirtualPad();
  void computeCoefficients(std::vector<double> &coef,
                           std::vector<double> &prob) const;
  double mlem(const std::vector<double> &coef, const std::vector<double> &prob,
              int nIter);
  void findCOG(const TH2D &histMLEM, double xy[2]) const;
  void refinePixelArray(const double xyCOG[2], size_t nPixMax, double &xMin,
                        double &xMax, double &yMin, double &yMax);
  void cleanPixelArray(double threshold, std::vector<double> &prob);

  int fit(const std::vector<const std::vector<int> *> &clustersOfPixels,
          const double fitRange[2][2], double fitParam[SNFitParamMax + 1]);
  double fit(double currentParam[SNFitParamMax + 2],
             const double parmin[SNFitParamMax],
             const double parmax[SNFitParamMax], int nParamUsed,
             int &nTrials) const;
  double computeChi2(const double param[SNFitParamMax + 2],
                     int nParamUsed) const;
  void param2ChargeFraction(const double param[SNFitParamMax], int nParamUsed,
  ...
  void addPixel(const TH2D &histMLEM, int i0, int j0, std::vector<int> &pixels,
                std::vector<std::vector<bool>> &isUsed);
  void addCluster(int iCluster, std::vector<int> &coupledClusters,
                  std::vector<bool> &isClUsed,
                  const std::vector<std::vector<double>> &couplingClCl) const;
  void extractLeastCoupledClusters(
      std::vector<int> &coupledClusters, std::vector<int> &clustersForFit,
      const std::vector<std::vector<double>> &couplingClCl) const;
  int selectPads(const std::vector<int> &coupledClusters,
                 const std::vector<int> &clustersForFit,
                 const std::vector<std::vector<double>> &couplingClPad);
  void merge(const std::vector<int> &clustersForFit,
  ...
  std::function<double(uint32_t)> mADCToCharge = [](uint32_t adc) {
  ...
  MathiesonOriginal *mMathieson =
      nullptr; ///< pointer to the Mathieson function currently used

  std::unique_ptr<ClusterOriginal>
      mPreCluster; ///< precluster currently processed
  std::vector<PadOriginal>
      mPixels; ///< list of pixels for the current precluster
