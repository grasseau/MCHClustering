...
void printArrayState(const char *str, const double *array, int K, int N) {
  printf("%s\n min/max/sum %15.9g, %15.9g, %15.9g\n", str,
         vectorMin(array, K * N), vectorMax(array, K * N),
         vectorSum(array, K * N));
}
...
void computeDiscretizedGaussian2D(const double *xyInfSup, const double *theta,
  ...
  // Compute
  /*
    ISupSup[:] = 0.25*( 1.0 + vErf( (xsup[:]-muX) * cstx ) ) * ( 1.0 + vErf(
    (ysup[:]-muY) * csty ) ) IInfInf[:] = 0.25*( 1.0 + vErf( (xinf[:]-muX) *
    cstx ) ) * ( 1.0 + vErf( (yinf[:]-muY) * csty ) ) IInfSup[:] = 0.25*( 1.0 +
    vErf( (xinf[:]-muX) * cstx ) ) * ( 1.0 + vErf( (ysup[:]-muY) * csty ) )
    ISupInf[:] = 0.25*( 1.0 + vErf( (xsup[:]-muX) * cstx ) ) * ( 1.0 + vErf(
    (yinf[:]-muY) * csty ) )
    */
  double IxInf[N];
  double IyInf[N];
...
double computeWeightedLogLikelihood(const double *xyInfSup, const double *theta,
                                    const double *z, int K, int N) {
  // w is assumed to be normalized
  double li[N];
  double LogLikelihood = 0;

  generateMixedGaussians2D(xyInfSup, theta, K, N, li);
  // Avoiding computing log( small values ) replace by 0.0 (meaning no
  // contribution)
  for (int i = 0; i < N; i++) {
...
void EStep(const double *xyInfSup, const double *theta, int K, int N,
           int verbose, double *eta, double *zEval) {
  /*
...
*/
  ...
  for (int k = 0; k < K; k++) {
    // eta[:, k] = w[k] * computeDiscretizedGaussian2D( xy[:,:], dxy, mu[k,:],
    // var[k,:] ) kSum += eta[:,k]
    if (w[k] != 0.0) {
  ...
  for (int i = 0; i < N; i++) {
    if (kSum[i] < DBL_EPSILON) {
      // print( "WARNING sum eta(i,:) is null, i=",i , "xy=", xy[:,i],
      // "dxy=",dxy[:,i]) eta[ i, :] = eta[i, :] / kSum[i]
      for (int k = 0; k < K; k++) {
        eta[k * N + i] = 0.;
      }
    } else {
...
void maskedEStep(const double *xyInfSup, const double *theta,
                 const Mask_t *maskTheta, int K, int N, int verbose,
                 double *eta, double *zEval) {
  /*
          Compute new eta(i,k), fraction/proba that point i
          belongs to kth gaussian
*/
...
void weightedMStep(const double *xyDxy, const double *z, const double *eta,
                   int K, int N, int cstVar, double *theta) {
  // xyDxy[N,4]
  // z[N]
  // eta[K, N]
  ...
  gsl_blas_dgemv(CblasNoTrans, 1.0, &gsl_eta.matrix, &gsl_z.vector, 0.0,
                 &gsl_wk.vector);
  ...
  gsl_blas_dgemv(CblasNoTrans, 1.0, &gsl_eta.matrix, &gsl_u.vector, 0.0,
                 &gsl_muX.vector);
  //
  //  Compute muY
  //
  ...
  if (cstVar == 0) {
    ...
    for (int k = 0; k < K; k++) {
      ...
      gsl_vector_const_view gsl_etaK =
          gsl_vector_const_view_array(&eta[k * N], N);
...
double weightedEMLoop(const double *xyDxy, const Mask_t *saturated,
                      const double *zObs, const double *theta0,
                      const Mask_t *maskTheta, int K, int N, int mode,
                      double LConvergence, int verbose, double *theta) {
  ...
  logL = 1.0 + prevLogL;
  int it = 0;
  // for( ; (( fabs((logL - prevLogL)/logL) > LConvergence) || (it < nIterMin))
  // && ( it < nIterMax ); ) {
  for (;
       ((fabs((logL - prevLogL) / logL) > LConvergence)) && (it < nIterMax);) {
    if (verbose >= 1)
  ...
  double AICc = 2 * (3 * kSignificant - 1) - 2 * logL +
                2 * dof * (dof + 1) / (nData - dof - 1);
  double zPredict[N];
...
double weightedEMLoopWithMuCriterion(const double *xyDxy,
                                     const Mask_t *saturated,
                                     const double *zObs, const double *theta0,
                                     const Mask_t *maskTheta, int K, int N,
                                     int mode, double LConvergence, int verbose,
                                     double *theta) {
  ...
  // for( ; (( fabs((logL - prevLogL)/logL) > LConvergence) || (it < nIterMin))
  // && ( it < nIterMax ); ) {
  for (; (muChanged == 0) && (fabs((logL - prevLogL) / logL) > LConvergence) &&
         (it < nIterMax);) {
    if (verbose >= 1)
      printEMState(it, logL, logL - prevLogL);
    prevLogL = logL;
  ...
  int kSignificant = vectorSumOfGreater(w, 10.e-5, K);
  // printf("EMLoop # parameters %d, log( N -saturated)= %f \n",
  // (3*kSignificant-1), log(N - nbrSaturatedPads) ); printf("EMLoop LogL=%f\n",
  // logL );
  double dof = (3 * kSignificant - 1);
  double nData = N - nbrSaturatedPads;
  double BIC = -2 * logL + (3 * kSignificant - 1) * log(N - nbrSaturatedPads);
