...
def findLocalMaxWithSubstraction( xi, yi, dxi, dyi, cathi, chi, ax=None, maxIter=200 ):
    x = []; dx = []
    y = []; dy = []
    ch = []
...
def displayMCTracks( mcObj, ev ):
    ...
    for chId in md.ChId:
      ...
      for t in range( len(mcObj.trackX[ev]) ):
        idx = (mcObj.trackChId[ev][t] == (chId-1) )
...
if __name__ == "__main__":
    ...
    for ev in range( len(mcData.padX)):
        getContributingMCTracks( mcData, ev)
        displayMCPads( mcData, ev )
    #displayMCPads( mcData, 5 )
